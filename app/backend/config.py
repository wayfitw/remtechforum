...
# Отбраковка вариантов, где модель дорисовала второго человека. Лицо считается
# «лишним», если детектор в нём уверен (морда медведя даёт ~0.54, люди 0.86+)
# и оно не меньше трети лица гостя — мелкие фигуры вдали кадр не портят.
EXTRA_PEOPLE_CHECK = os.environ.get("EXTRA_PEOPLE_CHECK", "1").strip() in ("1", "true", "yes")
EXTRA_FACE_MIN_SCORE = float(os.environ.get("EXTRA_FACE_MIN_SCORE", "0.7"))
EXTRA_FACE_MIN_RATIO = float(os.environ.get("EXTRA_FACE_MIN_RATIO", "0.35"))
