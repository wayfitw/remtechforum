...
def main():
    mark = build()
    # В ряду логотипов на карточке знак монохромный, как у партнёров: буквы
    # графитом, под ними «РЕМТЕХНИКА» капсом. Имя с подчёркивания — compositor
    # подставляет его сам, вне сортировки 0*.png.
    signature(mark, INK).save(CARD / "_rt_mono.png")
